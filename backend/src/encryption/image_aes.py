...
class ImageAes128:
    ...
    def decrypt_image_as_image(self, image: Image.Image) -> Image.Image:
        return Image.fromarray(self.decrypt_image_as_array(image), image.mode)  # type: ignore
    ...
    def encrypt_image_as_array(self, image: Image.Image) -> NDArray[np.uint8]:
        ...
        for row in range(0, h, 4):
            for col in range(0, w, 4):
                for d in range(3):
                    enc_image[row:row+4, col:col+4, d] = self._encrypt_block(img[row:row+4, col:col+4, d])
        return enc_image
    ...
    def decrypt_image_as_array(self, image: Image.Image) -> NDArray[np.uint8]:
        ...
        for row in range(0, h, 4):
            for col in range(0, w, 4):
                for d in range(3):
                    dec_image[row:row+4, col:col+4, d] = self._decrypt_block(img[row:row+4, col:col+4, d])
        return dec_image
    ...
    def _inv_mix_columns(self, state: NDArray[np.uint8]):
        def calc_inv_mix_cols(a0: np.uint8, a1: np.uint8, a2: np.uint8, a3: np.uint8) -> NDArray[np.uint8]:
            # 14*a0 + 11*a1 + 13*a2 +  9*a3
            r0 = consts.Gmul14[a0] ^ consts.Gmul11[a1] ^ consts.Gmul13[a2] ^ consts.Gmul9[a3] 
            #  9*a0 + 14*a1 + 11*a2 + 13*a3
            r1 = consts.Gmul9[a0] ^ consts.Gmul14[a1] ^ consts.Gmul11[a2] ^ consts.Gmul13[a3] 
            # 13*a0 +  9*a1 + 14*a2 + 11*a3
            r2 = consts.Gmul13[a0] ^ consts.Gmul9[a1] ^ consts.Gmul14[a2] ^ consts.Gmul11[a3] 
            # 11*a0 + 13*a1 +  9*a2 + 14*a3
            r3 = consts.Gmul11[a0] ^ consts.Gmul13[a1] ^ consts.Gmul9[a2] ^ consts.Gmul14[a3] 
    ...
    def _mix_columns(self,  state: NDArray[np.uint8]):
        def calc_mix_cols(a0: np.uint8, a1: np.uint8, a2: np.uint8, a3: np.uint8) -> NDArray[np.uint8]:
            r0 = consts.Gmul2[a0] ^ consts.Gmul3[a1] ^ a2 ^ a3 # 2*a0 + 3*a1 + a2   + a3
            r1 = a0 ^ consts.Gmul2[a1] ^ consts.Gmul3[a2] ^ a3 # a0   + 2*a1 + 3*a2 + a3
            r2 = a0 ^ a1 ^ consts.Gmul2[a2] ^ consts.Gmul3[a3] # a0   + a1   + 2*a2 + 3*a3
            r3 = consts.Gmul3[a0] ^ a1 ^ a2 ^ consts.Gmul2[a3] # 3*a0 + a1   + a2   + 2*a3
            return np.array([r0, r1, r2, r3], np.uint8)
